...
# [3] 그룹절과 집계함수, 그룹(~~별, ~~끼리)
SELECT mid, SUM(bamount) 총구매수량 FROM buy GROUP BY mid; -- 1) mid 기준으로 그룹하여 총구매수량 (bamount)
...
# SELECT mid, SUM(bamount) 총구매수량 FROM buy WHERE 총구매수량 >5 GROUP BY mid;
 -- [오류] where 절에서 필드의 별칭 사용이 안된다. 왜? where 먼저 처리

# [5] order by 정렬, desc 내림차순(3 2 1 , C B A), asc 오름차순
SELECT * FROM member ORDER BY mdebut desc;

# ** 다중정렬이란? 첫번째 정렬 후 첫번째 정렬 필드 기준으로 중복이 존재한 경우 중복끼리 2차정렬
-- 지역(maddr) 먼저 정렬하고 만약에 지역 필드내 동일한 값끼리 2차정렬(mdebut)한다.
SELECT*FROM member ORDER BY maddr DESC, mdebut ASC;

# [6] limit: 결과 레코드 제한, *페이징처리* 
SELECT * FROM member;
SELECT * FROM member LIMIT 5,3; -- 6~8출력 (5인덱스부터 3개)
