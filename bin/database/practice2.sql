...
CREATE TABLE members(
    member_id int AUTO_INCREMENT,constraint PRIMARY KEY(member_id), member_name VARCHAR(50) not null, 
    email VARCHAR(100) UNIQUE not null,join_date DATETIME DEFAULT now(), is_active BOOLEAN DEFAULT true
);
...
CREATE TABLE products(
    product_id int AUTO_INCREMENT, constraint PRIMARY KEY(product_id), product_name VARCHAR(100) not NULL, 
    price int UNSIGNED not NULL, stock int DEFAULT 0 not NULL, created_at DATETIME DEFAULT now()
);
...
CREATE Table order_items(
    item_id int AUTO_INCREMENT, constraint PRIMARY KEY(item_id), order_id bigint , constraint FOREIGN KEY(order_id) REFERENCES orders(order_id),
    product_id int, constraint FOREIGN KEY(product_id) REFERENCES products(product_id), quantity int DEFAULT 1, price int UNSIGNED not NULL
);
...
CREATE TABLE students(
    student_id VARCHAR(10), constraint PRIMARY KEY(student_id), student_name VARCHAR(30) not null , 
    major VARCHAR(50), grade TINYINT UNSIGNED, enrolled_date DATE
);
...
CREATE TABLE employees(
    emp_id int AUTO_INCREMENT, constraint PRIMARY KEY(emp_id), emp_name VARCHAR(40) not null ,
    salary int UNSIGNED not null, hire_date DATE not null, department VARCHAR(50)
)
...
CREATE Table boards(
    board_id int AUTO_INCREMENT, constraint PRIMARY KEY(board_id), title VARCHAR(200) not null,
    contents text not null, writer_id int, constraint FOREIGN KEY(writer_id) REFERENCES members(member_id), created_at DATETIME DEFAULT now()
)
...
-- created_at (작성일): datetime, Default 현재 날짜/시간
CREATE Table commentts(
    comment_id int AUTO_INCREMENT, constraint PRIMARY KEY(comment_id), board_id int,
    constraint FOREIGN KEY(board_id) REFERENCES boards(board_id) , writer_id int, constraint FOREIGN KEY(writer_id) REFERENCES members(member_id), 
    contents VARCHAR(300) not NULL, created_at DATETIME DEFAULT now()
)
...
CREATE Table payments(
    ...
    payment_amount int UNSIGNED not null, payment_method VARCHAR(30), payment_date DATETIME DEFAULT now()
)
...
CREATE Table reviews(
    ...
    member_id int, constraint FOREIGN KEY(member_id) REFERENCES members(member_id), 
    rating tinyint UNSIGNED not null, review_text text, created_at DATETIME DEFAULT now()
)