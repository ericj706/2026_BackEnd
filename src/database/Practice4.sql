...
SELECT customer, sum(order_qty) 총주문수량 FROM orders GROUP BY customer HAVING 총주문수량 >= 5;
-- [문제 13 ] orders에서 도서번호(book_id)별 주문 건수가 3회 이상인 도서만 조회하세요.( 출력: book_id, 주문건수 )
SELECT book_id, COUNT(order_qty) 주문수량 from orders GROUP BY book_id HAVING 주문수량 >=3;
...
SELECT price FROM books ORDER BY price desc LIMIT 0,3;